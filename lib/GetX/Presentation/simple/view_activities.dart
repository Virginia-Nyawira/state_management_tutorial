...
class ViewActivities extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: GetBuilder<SimpleControllerLogic>(
        builder: (controller) => _buildListView(controller),
      ),
    );
  }

  Widget _buildListView(SimpleControllerLogic controllerLogic) {
    return controllerLogic.myActivities.isEmpty
        ? const Center(
      child: Text(
        'No activities',
        style: TextStyle(fontSize: 20),
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          leading: Text(
            controllerLogic.myActivities[index].activity,
            style: labelStyle().copyWith(
              fontSize: 17.0,
            ),
          ),
          title: Text(
            controllerLogic.myActivities[index].date.toString(),
            style: labelStyle().copyWith(
              fontSize: 17.0,
            ),
          ),
          subtitle: Text(
            controllerLogic.myActivities[index].time.toString(),
            style: labelStyle().copyWith(
              fontSize: 17.0,
            ),
          ),
          trailing: IconButton(
            onPressed: () {
              if (controllerLogic.myActivities.contains(done)) {
                if (controllerLogic.doneActivities.contains(done)) {
                  // Show snackbar if item is already added
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('Item already added to done list'),
                    ),
